...
_, scs_error = packetHandler.WriteKpKd(1, 18, 1)
Kp, Kd, _, scs_error = packetHandler.ReadKpKd(1)
...
while 1:
    ...
    scs_comm_result, scs_error = packetHandler.WritePosEx(1, 3000, 1100, 5)
    if scs_comm_result != COMM_SUCCESS:
    ...
    scs_comm_result, scs_error = packetHandler.WritePosEx(1, 1100, 3000, 50)
    if scs_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(scs_comm_result))
    elif scs_error != 0:
        print("%s" % packetHandler.getRxPacketError(scs_error))
