...
class sms_sts(protocol_packet_handler):
    def __init__(self, portHandler):
        ...
        self.groupSyncWrite_KpKd = GroupSyncWrite(self, SMS_STS_P_GAIN, 2)
        self.groupSyncWrite_PWMTorque = GroupSyncWrite(self, SMS_STS_GOAL_TIME_L, 2)
        self.groupSyncWrite_TorqueOffCalPos = GroupSyncWrite(self, SMS_STS_TORQUE_ENABLE, 1) # 0 torque off, 1 torque on, 128 set motor pos to be 2048
    ...
    def WriteKpKd(self, scs_id, Kp, Kd):
        txpacket = [Kp, Kd]
        return self.writeTxRx(scs_id, SMS_STS_P_GAIN, len(txpacket), txpacket)
    
    def ReadKpKd(self, scs_id):
        scs_Kp_Kd, scs_comm_result, scs_error = self.read2ByteTxRx(scs_id, SMS_STS_P_GAIN)
        scs_Kp = self.scs_lobyte(scs_Kp_Kd)
        scs_Kd = self.scs_hibyte(scs_Kp_Kd)
        return scs_Kp, scs_Kd, scs_comm_result, scs_error
    
    def ReadCurrentLoad(self, scs_id): # Gets motor torque percentage used comared to max torque allowed
        scs_current_load, scs_comm_result, scs_error = self.read2ByteTxRx(scs_id, SMS_STS_PRESENT_LOAD_L)
        return self.scs_tohost(scs_current_load, 15), scs_comm_result, scs_error
    
    def ReadCurrentCurrent(self, scs_id):
        scs_current_current, scs_comm_result, scs_error = self.read2ByteTxRx(scs_id, SMS_STS_PRESENT_CURRENT_L)
        return self.scs_tohost(scs_current_current, 15), scs_comm_result, scs_error
    ...
    def SyncWritePWMTorque(self, scs_id, pwmtorque):
        pwm_word = self.scs_toscs(pwmtorque, 10)                  # 10th bit becomes the direction bit
        txpacket = [self.scs_lobyte(pwm_word), self.scs_hibyte(pwm_word)]
        return self.groupSyncWrite_PWMTorque.addParam(scs_id, txpacket)
    
    def SyncWriteKpKdEx(self, scs_id, kp, kd):
        txpacket = [kp, kd]
        return self.groupSyncWrite_KpKd.addParam(scs_id, txpacket)
    
    def SyncTorqueOffCalPos(self, scs_id, num):
        txpacket = [num]
        return self.groupSyncWrite_TorqueOffCalPos.addParam(scs_id, txpacket)
