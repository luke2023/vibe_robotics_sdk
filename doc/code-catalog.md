...
- `requirements.txt`: Python dependencies for simulation/control tooling.
- `LICENSE`: Apache 2.0 license.
- `doc/`: project documentation (setup notes, code catalog).
...
- `sundaya1_sim_config_leg_only.yaml`: leg-only MuJoCo sim configuration.
- `sundaya1_real_config.yaml`: hardware configuration for real SundayA1.
- `zero_position.csv`: joint zero positions for calibration/offsets.
- `__pycache__/`: bytecode caches.

### controller
- `controller_base.py`: abstract controller interface for sim/real deployments.
- `deploy_real.py`: wires real robot control loop, IMU integration, and policy/controller selection.
...
### sensor
- `imu_arduino.py`: Arduino IMU serial reader and orientation parsing utilities.
...
- `motor_controller_manager.py`: orchestration wrapper around motor controllers for group operations.
- `calibrate.py`: calibration utilities for zeroing and aligning servos.
...
- `assets/models/policy_leg_only.pt`: TorchScript policy for leg-only MuJoCo model.
- `assets/mujoco/`: MuJoCo scene/model XMLs, configs, and geometry assets for SundayA1 variants (STL/PART files).
